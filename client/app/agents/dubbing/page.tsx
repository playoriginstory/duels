...
export default function DubbingAgent() {
    ...
    if (selectedFile.type.startsWith("video/")) {
      try {
        ...
        if (duration > MAX_VIDEO_DURATION_SEC)
          return alert(
            `Video too long! Max ${MAX_VIDEO_DURATION_SEC / 60} minutes`
          );
      } catch {
  ...
  const handleSubmit = async () => {
    ...
    if (sizeMB > MAX_FILE_SIZE_MB) return alert(`File too large`);
      ...
      {result && (
        <div className="mt-4 p-4 border border-gray-300 rounded space-y-2 text-gray-600 bg-gray-100 shadow-sm">
          ...
          <p>
            ...
            {result.expected_duration_sec}s
          </p>
      ...
      {status?.status === "dubbed" && result?.dubbing_id && (status?.target_languages?.length ?? 0) > 0 && (
        <div className="mt-6 p-4 border border-gray-300 rounded bg-gradient-to-br from-gray-100 to-gray-200 shadow-lg">
          ...
          <p className="text-sm text-gray-500 mt-2">
            Your dubbed video is ready to play or download.
          </p>
        </div>
      )}

      {/* Completed but No Language */}
      {status?.status === "dubbed" &&
        (status?.target_languages?.length ?? 0) === 0 && (
          <p className="mt-4 text-sm text-red-400">
            Dubbing completed but no target language was registered.
